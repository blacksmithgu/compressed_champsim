...
void CheckPolicyStatus() {
    if ( ( get_cycle_count() - last_cycle_count) >= interval ) {
        ...
        for ( int i = 0 ; i < num_cores; i++ ) {
            if ( i == 0 ) {
                std::cout << setw(15) << "ROP/BOP/DIFF";
            }
            for ( int j = 0 ; j < num_cores; j++ ) {
                if ( i == 0 ) {
                    std::cout << setw(28) << "CORE " << j << "\t";
                }
            }
            if ( i == 0  ) {
                std::cout << endl;
            ...
            std::cout << setw(15) << i;
            for ( int j = 0 ; j < num_cores; j++ ) {
                std::cout << setw(9) << misses[i][j][ROP] << "/" << setw(9) << misses[i][j][BOP] << "/" << setw(9) << misses[i][j][ROP] - misses[i][j][BOP] << "\t";
            }
            std::cout << endl;
        }
        std::cout << endl;
        ...
        for ( int i = 0 ; i < num_cores; i++ ) {
            ...
            std::cout << setw(15) << i;
            for ( int j = 0 ; j < num_cores; j++ ) {
                std::cout << setw(9) << accesses[i][j][ROP] << "/" << setw(9) << accesses[i][j][BOP] << "/" << setw(9) << accesses[i][j][ROP] - accesses[i][j][BOP] << "\t";
            }
            std::cout << endl;
        }
        std::cout << endl;
        ...
        for ( int i = 0 ; i < num_cores; i++ ) {
            ...
            std::cout << setw(15) << i;
            for ( int j = 0 ; j < num_cores; j++ ) {
                std::cout << setw(9) << misses[i][j][ROP] * 100.0 / accesses[i][j][ROP] << "/" << setw(9) << misses[i][j][BOP] * 100.0 / accesses[i][j][BOP] << "/" << setw(9) << (misses[i][j][ROP] * 100.0 / accesses[i][j][ROP]) - (misses[i][j][BOP] * 100.0 / accesses[i][j][BOP]) << "\t";
            }
            std::cout << endl;
        }
        std::cout << endl;
        ...
        for ( int i = 0 ; i < num_cores; i++ ) {
            std::cout << "Thread: " << i << " Old Policy: " << policyStr(follower[i]);
            ...
            std::cout << " New Policy: " << policyStr(follower[i]) << endl;
            std::cout << " total[ROP]: " << total[ROP] << " total[BOP]:" << total[BOP] << " diff: " << int64_t(total[ROP] - total[BOP]) << endl;
        }
        for ( int i = 0 ; i < num_cores; i++ ) {
            for ( int j = 0 ; j < num_cores; j++ ) {
                misses[i][j][ROP] = 0;
...
void RandomSetSelection(SetType* sets, int32_t total_sets, int32_t num_threads, int32_t num_types, int32_t sets_to_sample) {
    std::cout << "Selecting " << sets_to_sample << " sets for " << num_threads << " threads for " << num_types << " types" << " from " << total_sets << " sets " << std::endl; 
...
class LDPT {
    ...
    LDPT(uint32_t _instances, uint32_t _sets, uint32_t _ways) : instances(_instances), sets(_sets), ways(_ways), hits(0), misses(0) {
        ...
        /*std::cout << "LDPT sets: " << sets << " assoc:" << ways << " instances: " << instances << std::endl;
        std::cout << "address_bits: " << address_bits
            << " set_bits: " << set_bits
            << " tag_bits: " << tag_bits << std::endl;
        std::cout << hex << "address_mask: " << address_mask
            << " set_mask: " << set_mask
            << " tag_mask: " << tag_mask << dec << std::endl;
*/
        get_storage_state();

#if 0
        for ( uint32_t i = 0 ; i < 10000 ;  ) {
            std::cout << hex << setw(10) << mask_address(i) << setw(10) << get_set(mask_address(i)) << setw(10) << get_tag(mask_address(i)) << dec << std::endl;
            i++;
            std::cout << hex << setw(10) << mask_address(i) << setw(10) << get_set(mask_address(i)) << setw(10) << get_tag(mask_address(i)) << dec << std::endl;
            i += sets -1;
        }
    ...
    void setThreshold(int32_t inst, int32_t inc, int32_t dec) {
        ...
        std::cout << " Setting threshold for instance : " << inst << " inc: " << inc_threshold[inst] << " dec: " << dec_threshold[inst] << std::endl;
    }
    ...
    void updateEntry(uint64_t sig, int32_t current_ld, uint32_t inst) {
        if ( inst >= instances ) {
            std::cout << " " << inst << " " << instances << std::endl;
            assert(0);
        }
    ...
    int32_t getMyInstanceId(int32_t thread_index, int32_t set_index) {
        ...
        if ( (thread_index < 0) || (thread_index >= num_cores) ) {
            std::cout << thread_index << " " << num_cores << std::endl;
            assert(0);
        }
        if ( set_index < 0 || set_index >= (int32_t)llc_sets ) {
            std::cout << set_index << " " << llc_sets << std::endl;
            assert(0);
        }
    ...
    void display_stats() {
        //std::std::cout << " ldpt hits: " << setw(12) << hits
         //   << " misses: " << setw(12) <<  misses
          //  << " hit-rate: " << setw(12) << fixed << setprecision(4) << hits * 100.0 / (hits + misses) << "%" << std::endl;
    }

    void get_storage_state() {
#if 0
        std::cout << setw(80) << "Number of LDPT entries: " << setw(20) << sets * ways << std::endl;
        std::cout << setw(80) << "Fields of LDPT" << std::endl; 
        int total_bits = 0;
        std::cout << setw(80) << "lru: " << setw(20) << log2(ways) << std::endl;
        total_bits += log2(ways);
        std::cout << setw(80) << "tag: " << setw(20) << tag_bits << std::endl;
        total_bits += tag_bits;
        std::cout << setw(80) << "valid: " << setw(20) << 1 << std::endl;
        total_bits += 1;
        int ldpt_entry  = 0;
        std::cout << setw(80) << "ldpt_entry[stable_ld]: " << setw(20) << log2(max_nru_bits+1) + 1<< std::endl;
        ldpt_entry += log2(max_nru_bits+1) + 1;
        std::cout << setw(80) << "ldpt_entry[variance_conf]: " << setw(20) << log2(max_conservative_conf+1) << std::endl;
        ldpt_entry += log2(max_conservative_conf+1);
        std::cout << setw(80) << "ldpt_entry[variance_dir]: " << setw(20) << 1 << std::endl;
        ldpt_entry += 1;
        std::cout << setw(80) << "LDPT entry size" << setw(20) << total_bits + ( ldpt_entry * instances)  << std::endl; 

        std::cout << setw(80) << "Total LDPT size ( bit) = " << setw(20) << (total_bits + (ldpt_entry * instances) ) * sets * ways << std::endl;
        std::cout << setw(80) << "Total LDPT size (kbytes) = " << setw(20) << (total_bits + (ldpt_entry * instances) ) * sets * ways  / ( 1024.0 * 8) << std::endl;
